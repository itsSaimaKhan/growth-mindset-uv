...
if files:
    for file in files:
        ...
        if st.checkbox(f"Fill Missing Values- {file.name}"):
            ...
            st.success("Missing values fill successfully!")
            st.dataframe(df.head())
            ...
            if st.checkbox(f"📊 Show Chart - {file.name}") and not df.select_dtypes(include="number").empty:
                ...
                format_choce = st.radio(f"Convert {file.name} to:", ["CSV", "Excel"], key=file.name)
                
                if st.button(f"⬇️ Download {file.name} as {format_choce}"):
                    output = BytesIO()
                    if format_choce == "CSV":
